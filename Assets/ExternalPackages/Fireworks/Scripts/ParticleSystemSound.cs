    ...
    public class ParticleSystemSound : MonoBehaviour
    {
        ...
        public void LateUpdate()
        {
            ...
            try
            {
                while (i < length)
                {
                    if (_explosionSound.Length > 0 && particles[i].remainingLifetime < Time.unscaledDeltaTime)
                    {
                    ...
                    if (_shootSound.Length > 0 && particles[i].remainingLifetime >= particles[i].startLifetime - Time.unscaledDeltaTime)
                    {
