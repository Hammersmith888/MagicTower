	...
	public class FX_ElectroLine : MonoBehaviour
	{
		...
		void Start( )
		{
			...
			if( FXEnd != null )
			{
				...
				if (disableLightning)
				{
					if (fxEnd.GetComponent<LightningShot>() != null)
						Destroy(fxEnd.GetComponent<LightningShot>());
				}
				if( Normal )
		...
		void UpdatePosition( )
		{
			if ( alignTransform && (EndPosition - StartPosition).normalized != Vector3.zero)
			{
